...
async def initialize_system():
    """
    🚀 Initialize Syria GPT Q&A System
    
    Loads all Syria knowledge data from the data folder into Redis cache and Qdrant vector database.
    ...
    try:
        ...
        if result.get("status") == "success":
            return {
                "status": "success",
                "data": result,
                "message": "Syria GPT Q&A system initialized successfully"
            }
        else:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def reload_knowledge_base():
    ...
    Reloads all Syria knowledge data from the data folder, clearing existing data first.
    Useful for updating the knowledge base with new data.
    """
    try:
        logger.info("Reloading knowledge base...")
        
        result = await intelligent_qa_service.reload_knowledge_base()
...
async def get_knowledge_base_statistics():
    ...
    Returns detailed statistics about the Syria knowledge base including:
